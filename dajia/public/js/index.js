...
var data = {
    zhong:{
        q1:{
            question: '如果《大家》的每一篇文章每被阅读一次,就给你一块钱,那你现在是什么级别的富人?',
            opitions:{
                A:'A : 亿万富翁',
                B:'B : 百万富翁',
                C:'C : 穷光蛋'
    ...
    img.onload = function () {
        ...
        if(percent == 100){
            setTimeout(function () {
                $('.loading').hide();
                $('.item-wrap').show();
                $('.cover-text').addClass('cover-text-animation')
                setTimeout(function (){
                    $('.cover-yun-1').addClass('cover-yun-1-ani');
...
var sex = null; //0为男,1为女
var level = null
$('.choose').on('click','.ac_sex',function(){ //选择男女点击事件
    var $t = $(this); //这里的this指的ac_choose
    //closest('li')返回第一个标签为li的祖先元素
    sex = $t.index();
    level = 0;
    var $li = $t.closest('li');
    var index = $li.index();
...
var levelArr = [
    ['太监','衙役','县令','宰相','皇帝'],
    ['丫头','小姐','小主','贵妃','女王'],
    ['taijian','yayi','xianling','zaixiang','huangdi'],
    ['yatou','xiaojie','xiaozhu','guifei','nvwang']
]
...
function renderchenghao(){ //渲染称号
    $('.main .level').text(levelArr[sex][level])
}
function randerQue(){ //渲染问题
    if (qNum <= qAll) {
        $('.answer .answer-box .title').text(data[name]['q'+ qNum]['question']) 
        //each的用法:each是遍历的意思,后面可能会跟一个函数,函数里面的两个形参是index和item,
        //他们的含义是index:正在遍历的元素的索引值,item:正在被遍历的元素的dom节点
        $('.answer .opt').each(function (index, item) { //渲染选项
            var map = {0:'A',1:'B',2:'C'} //传进来的index是012,通过map映射更改为ABC
            $item = $(item);
            $item.text(data[name]['q'+qNum]['opitions'][map[index]])
        })
        $('.thisNum').text(qNum);
        $('.allNum').text(qAll);
    }
}
function isAllOpen(){ //判断是否全部解锁
    if(getDaoju.length == 5){
        $('.allopen').velocity('fadeIn',{
            delay:20
        })
        setTimeout(function(){
            $('.allopen').velocity('fadeOut',{
                delay:20
            })
        },1500)
        setTimeout(function(){
            $('.zouzhang').velocity('fadeIn',{
                delay:400
            })
        },1700)
...
function remain(){ //回到主页面
    setTimeout(function(){
        $('.item-wrap ul').css({
            'transform':'translateY(' + ( - $(window).height() * 2) + 'px)' //ul位置复原
        });
        setTimeout(function(){
            $('.answer .result-right').hide(); //结果页-正确-复原
            $('.answer .result-wrong').hide(); //结果页-错误-复原
            $('.item-wrap ul .answer .answer-box').removeClass('blur'); //模糊效果复原
        },400)
    },1400)
    qNum = null;
    rightNum = 0;
...
function renwuRemain(){ //回到主页面
    setTimeout(function(){
        $('.item-wrap ul').css({
            'transform':'translateY(' + ( - $(window).height() * 2) + 'px)' //ul位置复原
        });
        setTimeout(function(){
            $('.answer .result-renwu-box').hide(); //结果页-复原
        },400)
    },2000)
    qNum = null;
    rightNum = 0;
...
$('.main').on('click','.ac_choose',function () {
    var $t = $(this); //这里的this指的ac_choose
    //closest('li')返回第一个标签为li的祖先元素
    var $li = $t.closest('li');
    var index = $li.index();
...
var lock = true; //防止连续点击
$('.answer .answer-box .result-opitions').on('click','.opt',function (){ //点击选项
    if (lock) { //锁
        lock = false; //上锁
        var opt = $('.answer .answer-box .result-opitions .opt');
        var $o = $(this);
        var index = $o.index();
        var right = data[name]['q'+qNum]['correct'];

        opt.eq(right).append('<div class="ans-right"></div>'); //添加对号

        qNum++;
        if (index == right) { //答对
            $o.addClass('opt-w'); //改变选项的颜色
            rightNum++;
        } else { //答错
            $o.append('<div class="ans-wrong"></div>'); //添加×
            $o.addClass('opt-w'); //改变选项颜色
        }
        setTimeout(function(){ //0.5秒后去掉对号错号
            opt.removeClass('opt-w');
            opt.children('div').remove();
            if (qNum != (qAll+1)) {
                randerQue();
            }else{
                debugger
                if (type != 'renwu') {
                    if (rightNum > 1) {
                        ...
                        $('.answer .result-right').velocity('fadeIn',{ //出现
                            delay:400,
                            begin:function() { //在动画开始的时候执行
                                $('.item-wrap ul .answer .answer-box').addClass('blur'); //添加模糊
                            }
                        })
                        document.getElementsByName(name)[1].style.backgroundPosition = '0 100%';
                        if(getDaoju.indexOf(name) == -1){ //把道具名字加入getdaoju数组
                            getDaoju.push(name)
                        }
                        setTimeout(function(){ //显示道具
                            document.getElementsByName(name)[0].style.opacity = '1'
                        },2000)
                        remain();
                        setTimeout(function(){
                            isAllOpen()                            
                        ...
                        $('.answer .result-wrong').velocity('fadeIn',{ //出现
                            delay:400,
                            begin:function() { //在动画开始的时候执行
                                $('.item-wrap ul .answer .answer-box').addClass('blur'); //添加模糊
                            }
...
$('.zouzhang').on('click',function(){
    var $ul = $(this).closest('ul');
    $ul.css({'transform':'translateY(' + ( - $(window).height()) * 4 + 'px)'})
})
...
$('.close').on('click',function(){
    var $ul = $(this).closest('ul');
    $ul.css({'transform':'translateY(' + ( - $(window).height()) * 2 + 'px)'})
})